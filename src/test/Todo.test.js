...
test('renders without crashing', () => {
  ...
  const {getByTestId}=render(<Todo todo={todo} />, div);
...
test('Checkbox calls  when todo is clicked', () => {
  ...
  const wrapper = shallow(<Todo todo={todo} onChecked={doneTodo} />);
...
test('delete todo  when delete button clicked', () => {
  ...
  const wrapper = shallow(<Todo todo={todo} onDelete={deleteTodo} />);
